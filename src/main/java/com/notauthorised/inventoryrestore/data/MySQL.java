...
public class MySQL {
    ...
    public static void convertYAMLToMySQL() {
        ...
        for (int i = 0; i < backupLocations.size(); i++) {
            ...
            InventoryRestore.getInstance().getConsoleSender()
                    .sendMessage("[InventoryRestore] Converting the backup location " + logType.name());
            ...
            for (File UUIDBackup : uuidBackups) {
                ...
                for (File backup : backups) {
                    ...
                    mysql.setOffhand(yaml.getOffhand());
        ...
        InventoryRestore.getInstance().getConsoleSender()
                .sendMessage("[InventoryRestore] Conversion completed!");
    }
        ...
        try (Connection conn = getConnection()) {
            for (BackupTable table : BackupTable.values()) {
                String tableQuery = "CREATE TABLE IF NOT EXISTS " + table.getTableName() + " (" +
                        ...
                        "`offhand` LONGTEXT CHARACTER SET utf8 COLLATE utf8_general_ci," +
            ...
            addOffhandColumnToExistingTables(conn);
        }
    }

    private static void addOffhandColumnToExistingTables(Connection conn) {
        for (BackupTable table : BackupTable.values()) {
            try {
                String alter = "ALTER TABLE " + table.getTableName() + " ADD COLUMN `offhand` LONGTEXT CHARACTER SET utf8 COLLATE utf8_general_ci";
                try (Statement stmt = conn.createStatement()) {
                    stmt.executeUpdate(alter);
                }
            } catch (SQLException e) {
                if (e.getErrorCode() != 1060) { // 1060 = Duplicate column name
                    InventoryRestore.getInstance().getLogger().warning("Could not add offhand column to " + table.getTableName() + ": " + e.getMessage());
                }
            }
        }
    }
    ...
    private static void createIndices(Connection conn, String tableName) {
        ...
        try {
            ...
            try (Statement stmt = conn.createStatement()) {
                ...
                InventoryRestore.getInstance().getConsoleSender().sendMessage("[InventoryRestore] Created index: " + indexName);
            }
        } catch (SQLException e) {
            if (e.getErrorCode() == 1061) {
                InventoryRestore.getInstance().getConsoleSender()
                        .sendMessage("[InventoryRestore] Index verified (exists): " + indexName);
            } else {
                InventoryRestore.getInstance().getLogger()
    ...
    public void saveData() throws SQLException {
        try (Connection conn = getConnection()) {
            ...
            String insert = "INSERT INTO " + backupTable.getTableName() + " " +
                    "(uuid, timestamp, xp, health, hunger, saturation, location_world, location_x, location_y, location_z, version, death_reason, main_inventory, armour, ender_chest, offhand) "
                    +
                    "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            ...
            try (PreparedStatement stmt = conn.prepareStatement(query)) {
                ...
                try (ResultSet rs = stmt.executeQuery()) {
                    if (rs.next()) {
                        ...
                        try {
                            offhand = rs.getString("offhand");
                        } catch (SQLException e) {
                            offhand = null; // Column may not exist in older DBs
                        }
                        xp = rs.getFloat("xp");
    ...
    public ItemStack getOffhand() {
        if (offhand == null || offhand.isEmpty()) return null;
        ItemStack[] arr = RestoreInventory.getInventoryItems(packageVersion, offhand);
