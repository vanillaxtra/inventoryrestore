...
public class LegacyBackupConversionUtil {
    ...
    public static void convertOldBackupData() {
        Logger logger = InventoryRestore.getPluginLogger();

        List<File> oldBackupTypeFolders = new ArrayList<>();

        File oldBackupsRoot = new File(ConfigData.getFolderLocation().getParentFile(), "InventoryRollback/saves"); // Legacy import path
        ...
        InventoryRestore.getPluginLogger().info(MessageData.getPluginPrefix() + "Conversion completed!");
    }
    ...
    private static ArrayList<File> getFilesToConvert(File[] availableFiles) {
        ...
        for (File file : availableFiles) {
            ...
            try {
                ...
                InventoryRestore.getInstance().getLogger().severe(
            ...
            try {
                ...
                try {
                    ...
                    InventoryRestore.getInstance().getLogger().severe(
                            "An error occurred when trying to retrieve the player UUID from " + backupFile.getAbsolutePath() + "#" + timestampStr + "! " +
                                    "Please ask for help in the issues section of the InventoryRestore github page.");
                    ex.printStackTrace();
                    continue;
                }
                ...
                if (logTypeStoredString == null) {
                    InventoryRestore.getInstance().getLogger().severe(
                            "An error occurred when trying to retrieve the backup type of " + backupFile.getAbsolutePath() + "#" + timestampStr + "! " +
                                    "Please ask for help in the issues section of the InventoryRestore github page. (typeStr is null)");
                    continue;
                }
                ...
                if (logTypeStored == null) {
                    try {
                        ...
                        InventoryRestore.getInstance().getLogger().severe(
                                "An error occurred when trying to retrieve the backup type of " + backupFile.getAbsolutePath() + "#" + timestampStr + "! " +
                                        "Please ask for help in the issues section of the InventoryRestore github page. (typeStr: " + logTypeStoredString + ")");
                        continue;
                    }
                }
                ...
                InventoryRestore.getPluginLogger().warning(
